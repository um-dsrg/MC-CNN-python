...
def compute_cost_volume(featuresl, featuresr, ndisp):
    ...
    left_cost_volume = np.zeros([2*ndisp+1, height, width], dtype=np.float32)
    # Initialize the cost volume using the right image as reference
    right_cost_volume = np.zeros([2*ndisp+1, height, width], dtype=np.float32)
    
    for d in range(-ndisp,ndisp+1):
        ...
        left_cost_volume[d+ndisp,:,:] = np.sum(np.multiply(featuresl, featuresr_d), axis=2)
        right_cost_volume[d+ndisp,:,:] = np.sum(np.multiply(featuresr, featuresl_d), axis=2)
